...
install_pkg() {
    ...
    sudo $PKG_INSTALL bun || { msg_err "Installation failed"; exit 1; }
    
    msg_ok "bun installed successfully!"
}
...
update_pkg() {
    ...
    sudo $PKG_UPDATE || true
    sudo $PKG_INSTALL bun || { msg_err "Update failed"; exit 1; }
    
    msg_ok "bun updated successfully!"
}

uninstall_pkg() {
    msg_info "Uninstalling bun..."
    detect_os
    
    sudo $PKG_UNINSTALL bun || { msg_err "Uninstallation failed"; exit 1; }
    
    msg_ok "bun uninstalled successfully!"
}
